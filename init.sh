...
# Add seeds and peers
PEERS="$(curl -sS https://initia-t-rpc.syanodes.my.id/net_info | jq -r '.result.peers[] | "\(.node_info.id)@\(.remote_ip):\(.node_info.listen_addr)"' | awk -F ':' '{print $1":>
...
# Set min gas price, chain and keyring
sed -i -e "s/^minimum-gas-prices *=.*/minimum-gas-prices = \"0.15uinit,0.01uusdc\"/" $HOME/.initia/config/app.toml
initiad config set client chain-id
